...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-65 bg-[#1a1d2e] border-r border-white/5 flex flex-col transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 ${
            ...
            {mainNavItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  to={item.path}
                  onClick={() => { if (isSidebarOpen) toggleSidebar(); }}
                  className={`relative flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-semibold transition-all duration-200 select-none group ${
                    isActive
                ...
                >
                  ...
                  <Icon className={`w-5 h-5 shrink-0 transition-transform duration-200 group-hover:scale-105 ${
                    isActive ? 'text-primary-hover' : 'text-slate-500 group-hover:text-slate-300'
                  }`} />
